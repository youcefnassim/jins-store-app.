...
function TrackOrderContent() {
  ...
  const timeline = [
    { label: t("timeline.created"), index: 0 },
    { label: t("timeline.verification"), index: 1 },
    { label: t("timeline.processing"), index: 2 },
    { label: t("timeline.completed"), index: 3 },
  ];
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-12 md:py-20">
      <div className="max-w-3xl mx-auto">
            ...
            >
              <div className="relative flex-1">
                ...
                <Input 
                  ...
                  className="pl-11 rtl:pl-4 rtl:pr-11 h-14 bg-black/5 dark:bg-black/40 border-black/10 dark:border-white/10 text-lg text-slate-900 dark:text-white placeholder:text-muted-foreground/50"
                />
              </div>
              <Button type="submit" disabled={loading} className="h-14 px-8 text-base bg-slate-900 text-white dark:bg-primary dark:text-primary-foreground hover:bg-slate-800 dark:hover:bg-primary/90">
        ...
        {error && (
          <div className="p-6 bg-destructive/10 border border-destructive/20 rounded-xl flex items-start gap-4 text-destructive-foreground rtl:text-right">
            <AlertCircle className="w-6 h-6 text-destructive shrink-0" />
            <div className="text-left rtl:text-right">
              <h3 className="font-bold text-destructive text-lg mb-1">{t("not_found_title")}</h3>
              ...
              <CardContent className="p-6 md:p-10">
                {order.status === 'rejected' ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-2xl font-bold text-red-500 mb-2">{t("payment_rejected")}</h3>
                          ...
                          <div key={index} className="flex md:flex-col items-center gap-4 md:text-center">
                            ...
                            <div>
                              <p className={cn(
                                ...
                                isCompleted ? "text-slate-900 dark:text-white" : "text-muted-foreground"
                              )}>{step.label}</p>
                              {isCurrent && (
                                <p className="text-xs text-primary font-medium mt-1">{t("in_progress")}</p>
                              )}
                            </div>
                          </div>
                        );
